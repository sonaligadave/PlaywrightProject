<img width="885" height="964" alt="Screenshot 2026-04-08 at 12 00 40 AM" src="https://github.com/user-attachments/assets/297b5959-a624-4055-a535-4e63727cad8e" />

To start with 
markdown
# PlaywrightProject 

A brief description of what your Python project does.

## 🚀 Getting Started

Follow these steps to set up and run the project on your local machine.

### 1. Prerequisites
Make sure you have **Python 3.x** installed. You can check by running:
```bash    
python --version
Use code with caution.

2. Clone the Repository
bash
git clone https://github.com
cd your-repo-name
Use code with caution.

3. Create a Virtual Environment
It is recommended to use a virtual environment to keep dependencies isolated.
Windows:
bash
python -m venv venv
.\venv\Scripts\activate
Use code with caution.

macOS/Linux:
bash
python3 -m venv venv
source venv/bin/activate
Use code with caution.

4. Install Dependencies
Install the required packages using the requirements.txt file:
bash
pip install -r requirements.txt
Use code with caution.

5. Run the Project
Launch the application by running the main script (replace main.py with your actual file name):




playwright codegen https://www.saucedemo.com/
**To run automation on comanad line **

1. pytest
2. pytest --alluredir=allure-results
3. allure-results --clean -o allure-report allure open allure-report

**To run the tests->** pytest 

**To open the report ->** allure generate allure-results --clean -o allure-report

**Check the report loccaly ->** allure generate allure-results --clean -o allure-report allure open allure-report
                  
